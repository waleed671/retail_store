...
<div class="max-w-md">
    <div class="flex items-center gap-3 mb-6 anim-up">
        ...
        <h2 class="text-lg font-black" style="background:linear-gradient(135deg,#6366f1,#06b6d4);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;">Add Unit</h2>
    </div>

    <form action="{{ route('admin.units.store') }}" method="POST" class="glass-card p-6 rounded-2xl anim-up d1 space-y-5">
        @csrf
        <div>
            <label class="block text-xs font-semibold text-gray-500 mb-1">Unit Name <span class="text-red-400">*</span></label>
            <input type="text" name="name" value="{{ old('name') }}" class="input-cyber" placeholder="e.g. Pieces, Kilogram, Box" required>
        </div>
        <div>
            <label class="block text-xs font-semibold text-gray-500 mb-1">Abbreviation <span class="text-red-400">*</span></label>
            <input type="text" name="abbreviation" value="{{ old('abbreviation') }}" class="input-cyber" placeholder="e.g. Pcs, Kg, Box" required>
        </div>
